...
from models.STSSL.aug import sim_global
...
class TemporalHeteroModel(nn.Module):
    ...
    def __init__(self, c_in, batch_size, num_nodes, device):
        ...
        self.W1 = nn.Parameter(torch.FloatTensor(num_nodes, c_in)).to(device)  # representation weights
        self.W2 = nn.Parameter(torch.FloatTensor(num_nodes, c_in)).to(device)
...
class STEncoder(nn.Module):
    ...
    def _cheb_polynomial(self, laplacian, K):
        ...
        """
        N = laplacian.size(0)
        multi_order_laplacian = [torch.zeros([N, N], device=laplacian.device, dtype=torch.float) for _ in range(K)]
        ...
        if K == 1:
            ...
            if K == 2:
                ...
                for k in range(2, K):
                    # multi_order_laplacian[k] = torch.mm(multi_order_laplacian[k - 1], multi_order_laplacian[k - 1])
                    multi_order_laplacian[k] = 2 * torch.mm(laplacian, multi_order_laplacian[k - 1]) - \
                                               multi_order_laplacian[k - 2]
        return torch.stack(multi_order_laplacian, dim=0)
